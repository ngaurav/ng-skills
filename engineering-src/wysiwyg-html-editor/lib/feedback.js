...
(function () {
  ...
  // status.json (in-flight task messages, keyed by comment_id) — see fetchStatus
  let currentStatus = {};
  ...
  // Real agent edits involve multiple tool calls (read, edit, append history,
  // rebuild) and downstream skills/browser/web work can run several minutes, so
  // give a generous grace period before crying wolf. We also push this timer
  // back on any proof of life — history.json changing OR a live /status entry
  // for one of our in-flight comments — so the deadline only fires when the
  // batch is genuinely untouched by any agent.
  ...
  function renderPending() {
    ...
    if (lastSubmittedBatch) {
      ...
      if (isBatchStale) {
        ...
        // If the agent has posted a per-comment status string for any of our
        // in-flight ids, surface it inline. Multiple in-flight comments could
        // each have their own status — show one line per active status. This
        // is the only place currentStatus is consumed in the UI; if the
        // server-side feature isn't deployed, status.json is absent and this
        // section degrades silently to the original banner.
        const statusMsgs = lastSubmittedBatch.comment_ids
          .map(cid => (currentStatus[cid] && currentStatus[cid].message) || null)
          .filter(Boolean);
        const statusBlock = statusMsgs.length
          ? `<div class="cf-processing-status-msg">${statusMsgs.map(escapeHtml).join("<br>")}</div>`
          : "";
        banner.innerHTML = `
          <div class="cf-processing-row">
  ...
  // status.json polling — chained off the same interval as history.json so we
  // don't add a second poller. Optional decoration on the existing processing
  // banner: if an agent has written a status string for one of our in-flight
  // comments, we append it to the banner so the user sees "Claude is
  // processing… <what specifically>". Tolerates missing file, parse errors,
  // and stale entries (server prunes at 10min). Never used to determine "is
  // this resolved" — history.json remains source of truth for that.
  async function fetchStatus() {
    if (!lastSubmittedBatch) return; // nothing in flight, skip
    try {
      const resp = await fetch(STATUS_URL + "?t=" + Date.now());
      if (!resp.ok) {
        if (lastStatusString !== "") { lastStatusString = ""; currentStatus = {}; renderPending(); }
        return;
      }
      const text = await resp.text();
      const changed = text !== lastStatusString;
      if (changed) {
        lastStatusString = text;
        try {
          const parsed = JSON.parse(text);
          currentStatus = (parsed && typeof parsed === "object" && !Array.isArray(parsed)) ? parsed : {};
        } catch (_) {
          currentStatus = {};
        }
      }
      // A live status for one of our in-flight comments is proof an agent is
      // working — push the stale deadline back on EVERY poll, not just when the
      // status text changes. A long task posts one status line and then goes
      // quiet; the old code reset the timer only on a content change, so the
      // banner still flipped ~STALE_AFTER_MS after that single post even though
      // the agent was alive. We treat a status as "live" only within the
      // server's 10-min prune window, so a crashed agent that left an entry
      // behind still eventually surfaces the warning.
      const nowSec = Date.now() / 1000;
      const liveStatus = lastSubmittedBatch.comment_ids.some((cid) => {
        const e = currentStatus[cid];
        return e && e.message && (!e.started_at_epoch || nowSec - e.started_at_epoch < 600);
      });
      let needRender = changed;
      if (liveStatus) {
        if (staleTimer) clearTimeout(staleTimer);
        staleTimer = setTimeout(() => {
          if (lastSubmittedBatch && !lastBatchProcessed()) {
        ...
        if (isBatchStale) { isBatchStale = false; needRender = true; }
      }
      if (needRender) renderPending();
    } catch (e) { /* network glitch */ }
  }
  ...
  // The change log (history.json) is shared by every page served from this
  // directory. A change belongs to the current page iff its anchor resolves in
  // this DOM — the only page signal available client-side. Tour and History
  // scope to these, so a page with 2 local changes shows "1/2", not the global
  // total across all pages.
  function pageChanges() {
    return flattenChanges().filter((ch) => findAnchorNode(ch.anchor || ch.id));
  }
  ...
  function renderHistory() {
    ...
    list.innerHTML = "";
    // Newest batch first. history.json is shared across every page served from
    // this directory, so show only changes anchored on THIS page, and skip
    // batches that produced nothing here.
    for (let i = history.length - 1; i >= 0; i--) {
      const b = history[i];
      const changes = (b.changes || []).filter((ch) => findAnchorNode(ch.anchor || ch.id));
    ...
    $("cf-tour").disabled = pageChanges().length === 0;
  }
  ...
  // ---------------- Tour ----------------
  // Tour walks the changes anchored on THIS page (history.json is shared across
  // every page served from this directory). The label shows N/M position within
  // this page's changes. Start position is the first on-page change of the
  // latest batch that touched this page — so after a fresh batch the tour drops
  // you straight onto the newest local content.
  function startTour() {
    const all = pageChanges();
    if (!all.length) return;
    let startIdx = 0;
    for (let i = history.length - 1; i >= 0; i--) {
      const ids = new Set((history[i].changes || []).map((c) => c.id));
      const idx = all.findIndex((c) => ids.has(c.id));
  ...
  function init() {
    ...
    fetchHistory().then(() => {
      if (shouldAutoTour && pageChanges().length > 0) {
        setTimeout(startTour, 250);
      }
    });
    pollTimer = setInterval(() => { fetchHistory(); fetchStatus(); }, POLL_INTERVAL_MS);
...
/* =====================================================================
 * Table rendering — auto-enhances every <table> on the host page.
 *
 * Features injected automatically:
 *   1. Content-aware proportional column widths (browser measures, we lock)
 *   2. Sortable columns  (click any <th> in <thead>)
 *   3. Sticky first column (position: sticky, bg resolved at runtime)
 *
 * Column width algorithm:
 *   - Read the browser's own natural column widths (table-layout: auto)
 *   - Convert to proportions, then to percentages
 *   - Apply via <colgroup> with table-layout: fixed; width: 100%
 *   - Adaptive floor = max(60px, containerWidth / numCols) — prevents
 *     slivers on many-column tables; scaled down uniformly if sum overflows
 *
 * Idempotent — safe to run multiple times.
 * Skips tables inside #claude-feedback-root.
 * ===================================================================== */
(function () {
  function enhanceTables() {
    document.querySelectorAll("body table").forEach(function (table) {
      if (table.closest("#claude-feedback-root")) return;
      if (table.dataset.cfTableEnhanced) return;
      table.dataset.cfTableEnhanced = "1";

      // ---- 1. Content-aware proportional column widths -----------------
      applyProportionalWidths(table);

      // ---- 2. Sticky first column (background-aware) -------------------
      const stickyBg = (function resolveBg() {
        let el = table;
      ...
      table.classList.add("cf-sticky-col");
      if (stickyBg) table.style.setProperty("--cf-sticky-bg", stickyBg);

      // ---- 3. Sortable columns -----------------------------------------
      const thead = table.querySelector("thead");
      if (thead && thead.querySelectorAll("th").length > 0) {
        table.classList.add("cf-sortable");
        thead.querySelectorAll("th").forEach(function (th, colIdx) {
          th.addEventListener("click", function () { sortByColumn(table, colIdx, th); });
        });
      }
    });
  }

  function applyProportionalWidths(table) {
    const thead = table.querySelector("thead");
    if (!thead) return;
    const ths = Array.from(thead.querySelectorAll("th"));
    if (ths.length === 0) return;

    // Read the browser's own content-aware widths — table is still in
    // table-layout: auto at this point, so these reflect actual content needs.
    const naturalWidths = ths.map(function (th) { return th.getBoundingClientRect().width; });
    const totalNatural = naturalWidths.reduce(function (a, b) { return a + b; }, 0);
    if (totalNatural === 0) return;

    const proportions = naturalWidths.map(function (w) { return w / totalNatural; });

    // Container width & adaptive floor
    const containerWidth = (table.parentElement || table).getBoundingClientRect().width || 800;
    const numCols = ths.length;
    const floor = Math.max(60, containerWidth / numCols);

    // Apply floor, then scale down uniformly if the sum overflows the container
    let colPx = proportions.map(function (p) { return Math.max(floor, p * containerWidth); });
    const rawTotal = colPx.reduce(function (a, b) { return a + b; }, 0);
    if (rawTotal > containerWidth) {
      const scale = containerWidth / rawTotal;
      colPx = colPx.map(function (w) { return w * scale; });
    }

    // Inject <colgroup> with percentage widths and lock the layout
    const grandTotal = colPx.reduce(function (a, b) { return a + b; }, 0);
    ...
    colgroup.innerHTML = "";
    colPx.forEach(function (px) {
      const col = document.createElement("col");
      col.style.width = (px / grandTotal * 100).toFixed(2) + "%";
  ...
  function sortByColumn(table, colIdx, th) {
    ...
    const wasAsc = th.classList.contains("cf-sort-asc");

    table.querySelectorAll("th").forEach(function (h) {
      h.classList.remove("cf-sort-asc", "cf-sort-desc");
    });
    ...
    rows.sort(function (a, b) {
      ...
      const toNum = function (s) {
        const n = parseFloat(s.replace(/,/g, "").replace(/[KkMmBb]$/, function (m) {
          return m.toLowerCase() === "k" ? "e3" : m.toLowerCase() === "m" ? "e6" : "e9";
        }).replace(/[^0-9.eE+-]/g, ""));
        return isNaN(n) ? null : n;
      };
      const aN = toNum(aText), bN = toNum(bText);
      let cmp = (aN !== null && bN !== null)
        ? aN - bN
        : aText.localeCompare(bText, undefined, { numeric: true, sensitivity: "base" });
      return wasAsc ? -cmp : cmp;
    });

    rows.forEach(function (row) { tbody.appendChild(row); });
    th.classList.add(wasAsc ? "cf-sort-desc" : "cf-sort-asc");
  }

...
2026-09-10 — 2.4.0. Added a fourth Step 1 question: which assets are imported
or captured rather than generated (screenshots, screen recordings, logos, a
CSV). Surfaced by a real case — a video storyboard estimated durations for
three beats that were actually going to be a raw screen-captured demo. The
estimate was wrong on two counts: the footage didn't exist yet, and even once
shot, editing (cuts, speed-ups, held pauses) would move the number again. The
fix generalizes past that one video: a real customer logo, a screenshot dropped
into a slide, a CSV shown on screen are the same category of thing — a fixed
input whose content or duration is discovered, not authored — so the question
belongs in Step 1 (format-agnostic) rather than bolted onto video's duration
field alone. Added an Asset field to the beat card, a "raw asset — duration
TBD" exception to Step 3's duration guidance, and an "Assets to import or
shoot" section to the storyboard.md template.
